...
import { setProviderForTesting } from './ai/extraction.ts';
import type { AiProvider } from './ai/provider.ts';
import { createReplyStreamScanner } from './ai/replyStream.ts';
import { pool } from './db/pool.ts';
import { attachSocketServer } from './socket.ts';
import { closeDatabase, resetDatabase, seedTenant, type SeededTenant } from './test/helpers.ts';

const app = createApp();
let httpServer: HttpServer;
let port: number;
let ioServer: ReturnType<typeof attachSocketServer>;
...
const fakeProvider = (raw: string): AiProvider => ({
  name: 'stub',
 ...
 */
const streamingProvider = (raw: string, chunkSize = 7): AiProvider => ({
  name: 'stub',
  isDeterministic: false,
  supportsStreaming: true,
  // eslint-disable-next-line @typescript-eslint/require-await -- matches the async interface
...
const newSession = async (token: string): Promise<string> => {
  const response = await request(app)
...
const connect = (token: string | undefined): Promise<ClientSocket> =>
...
const waitFor = <T>(client: ClientSocket, event: string, timeoutMs = 8000): Promise<T> =>
  new Promise((resolve, reject) => {
...
describe('message round trip', () => {
  it('echoes the user message and then the assistant reply', async () => {
    ...
    const received = waitFor<{ message: { content: string } }>(
      client,
      SOCKET_EVENTS.MESSAGE_RECEIVED,
    );
    const replied = waitFor<{ reply: { kind: string; text: string } }>(
      client,
      SOCKET_EVENTS.ASSISTANT_REPLY,
    );
  ...
  it('emits a typing indicator before the reply', async () => {
    ...
    const typing = waitFor<{ typing: boolean }>(client, SOCKET_EVENTS.ASSISTANT_TYPING);
  ...
  it('rejects an empty message with a validation error', async () => {
    ...
    const failure = waitFor<{ code: string }>(client, SOCKET_EVENTS.ERROR);
...
describe('streaming the reply', () => {
  it('delivers the prose in fragments that add up to the final reply', async () => {
    ...
    const replied = waitFor<{ turnId: string; reply: { text: string } }>(
      client,
      SOCKET_EVENTS.ASSISTANT_REPLY,
    );
    const deltas = collect<{ turnId: string; delta: string }>(
      client,
      SOCKET_EVENTS.ASSISTANT_DELTA,
      replied,
    );
  ...
  it('streams prose only — never the JSON around it', async () => {
    ...
    const replied = waitFor(client, SOCKET_EVENTS.ASSISTANT_REPLY);
    const deltas = collect<{ delta: string }>(client, SOCKET_EVENTS.ASSISTANT_DELTA, replied);
  ...
  it('sends no fragments at all for a provider that cannot stream', async () => {
    ...
    const replied = waitFor<{ reply: { text: string } }>(client, SOCKET_EVENTS.ASSISTANT_REPLY);
...
describe('the socket enforces the same rules as REST', () => {
  ...
  it('books through the same service, honouring business rules', async () => {
    ...
    const created = waitFor<{ appointment: { providerName: string } }>(
      client,
    ...
    const { rows } = await pool.query<{ source: string }>('SELECT source FROM appointments');
  ...
  it('still refuses a double booking made over the socket', async () => {
    ...
    const replied = waitFor<{ reply: { kind: string } }>(client, SOCKET_EVENTS.ASSISTANT_REPLY);
...
describe('multi-tab delivery', () => {
  it('delivers a reply to every tab the same user has open', async () => {
    ...
    const onTabTwo = waitFor<{ reply: { text: string } }>(tabTwo, SOCKET_EVENTS.ASSISTANT_REPLY);
