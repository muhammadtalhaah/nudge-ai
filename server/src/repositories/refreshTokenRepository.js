/**
 * Refresh token persistence. Only hashes are stored — see utils/tokens.js.
 */
...
/**
 * @param {{ userId: string, tokenHash: string, expiresAt: Date, userAgent?: string | null }} input
 */
 ...
 */
export const findByHash = async (executor, tokenHash) => {
 ...
 */
export const revokeById = async (executor, id) => {
...
export const revokeAllForUser = async (executor, userId) => {
  const { rowCount } = await executor.query(
 ...
 */
export const deleteExpired = async (executor) => {
  const { rowCount } = await executor.query(
