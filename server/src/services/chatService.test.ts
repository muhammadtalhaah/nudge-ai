...
import { ProviderError, type AiProvider } from '../ai/provider.ts';
import { pool } from '../db/pool.ts';
import { closeDatabase, resetDatabase, seedTenant, type SeededTenant } from '../test/helpers.ts';
import chatService from './chatService.ts';
import type { Caller } from './appointmentService.ts';
...
/** A provider that returns exactly the text a test wants. */
const fakeProvider = (raw: string | (() => never)): AiProvider => ({
  name: 'stub',
  isDeterministic: false,
  supportsStreaming: false,
  complete: async () => {
    if (typeof raw === 'function') raw();
    return { raw: raw as string, model: 'fake-model', promptTokens: 10, completionTokens: 5 };
  },
});
...
const say = (token: string, sessionId: string, content: string) =>
  request(app)
...
const callerFor = async (email: string): Promise<Caller> => {
  const { rows } = await pool.query<Caller>(
...
describe('conversational booking', () => {
  it('books when the model supplies every detail', async () => {
    ...
    const { rows } = await pool.query<{ count: string; source: string }>(
...
describe('model misbehaviour degrades to the form', () => {
  const badResponses: Array<[string, string]> = [
  ...
  it('keeps the user message even when extraction fails', async () => {
    ...
    const contents = history.body.data.messages.map((m: { content: string }) => m.content);
...
describe("the model cannot exceed the user's authority", () => {
  ...
  it('will not book a doctor belonging to another tenant', async () => {
    ...
    const { rows: providerRows } = await pool.query<{ full_name: string }>(
...
describe('ai interaction logging', () => {
  ...
  it('records the failure reason when output is unusable', async () => {
    ...
    const { rows } = await pool.query<{ outcome: string; error_message: string }>(
...
describe('conversation replay', () => {
  it('restores the structured reply payload from history', async () => {
    ...
    const assistantTurn = history.body.data.messages.find(
