...
import { withTransaction, pool, type Executor } from '../db/pool.ts';
import {
...
import userRepository, { type User } from '../repositories/userRepository.ts';
import { hashPassword, verifyPassword, wasteTimeLikeAVerification } from '../utils/password.ts';
import {
...
/** What a controller gets back. The refresh token is set as a cookie, never serialised. */
...
/**
 * The only shape of a user that ever leaves the service layer. Building it explicitly
 * (rather than deleting `passwordHash` from the full row) means a column added to the
 * table cannot silently start appearing in API responses.
 */
 ...
 */
const resolveDefaultBusiness = async (executor: Executor) => {
...
/**
 * Revoke every session for a user, then fail the request.
 *
 ...
 */
const revokeFamilyAndFail = async (userId: string, reason: string): Promise<never> => {
...
export const getById = async (userId: string): Promise<PublicUser> => {
