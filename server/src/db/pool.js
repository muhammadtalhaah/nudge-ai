 ...
 *
 * Repositories accept an executor — either the pool or an active transaction client — as
 * their first argument. That single convention is what lets a service compose several
 ...
 * checked-out client inside a transaction. Both satisfy this shape, which is the whole
 * reason a repository never needs to know which one it was handed.
 *
 * @typedef {object} Executor
 * @property {(sql: string, params?: readonly unknown[]) => Promise<import('pg').QueryResult>} query
 */
...
const collapse = (sql) => sql.replace(/\s+/g, ' ').trim().slice(0, 200);
