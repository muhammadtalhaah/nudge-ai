 ...
 * the reply is read off an SSE body. Both paths converge on the same completion result, so
...
import { createReplyStreamScanner } from '../replyStream.js';
...
const callOnce = async (request, stream) => {
  const messages = [
 ...
 * @param {import('../provider.js').ReplyDeltaHandler} onReplyDelta
 * @returns {Promise<import('../provider.js').CompletionResult>}
 */
const consumeStream = async (response, onReplyDelta) => {
