...
import { CHAT_INTENTS, type ChatIntent } from '@shared/constants.ts';
import type { BookingField } from '@shared/constants.ts';

import { resolveDate, resolveTime } from '../naturalDate.ts';
import type {
  AiProvider,
  CompletionContext,
  CompletionRequest,
...
const SYMPTOM_TO_SPECIALTY: Array<[RegExp, string]> = [
 ...
 */
const detectIntent = (text: string, hasBookingSignal: boolean): ChatIntent => {
...
const detectSpecialty = (text: string, context: CompletionContext): string | null => {
  const lower = text.toLowerCase();
...
const detectProviderName = (text: string, context: CompletionContext): string | null => {
  const lower = text.toLowerCase();
 ...
 */
const composeReply = (intent: ChatIntent, missing: BookingField[]): string => {
...
export const createStubProvider = (): AiProvider => ({
  ...
  async complete(request: CompletionRequest): Promise<CompletionResult> {
     ...
     */
    const newestFirst = (fromCurrent: string | null, fromHistory: string | null): string | null =>
