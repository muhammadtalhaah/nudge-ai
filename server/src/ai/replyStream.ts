...
type Phase =
  /** Walking the document looking for the target key at the top level. */
  | 'seeking'
  /** Just closed a string; waiting to see whether a `:` makes it a key. */
  | 'after-string'
  /** The key matched; waiting for the opening quote of its value. */
  | 'expect-value'
  /** Inside the value, decoding it. */
  | 'capturing'
  /** The value ended, or turned out not to be a string. Nothing more will be emitted. */
  | 'done';

export interface ReplyStreamScanner {
  /** Feed the next chunk; returns whatever became readable, or '' if nothing did. */
  push(chunk: string): string;
  /** True once the value has been fully read (or established to be unreadable). */
  isDone(): boolean;
}

/**
 * @param key the object key whose string value should be streamed out
 */
export const createReplyStreamScanner = (key = 'reply'): ReplyStreamScanner => {
  /**
