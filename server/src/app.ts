 ...
 *
 * Kept separate from server.ts (which owns the HTTP listener, sockets and shutdown) so
...
import { globalRateLimit } from './middlewares/rateLimit.ts';
...
export const createApp = (): Express => {
  ...
  app.use(
    pinoHttp({
      ...
      // only to satisfy pino-http's non-optional return type.
