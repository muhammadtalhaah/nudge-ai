...
import { sendMessageSchema } from '@shared/schemas.ts';

import { env } from './config/env.ts';
import { pool } from './db/pool.ts';
import { AppError } from './errors/AppError.ts';
import { socketLogger } from './logger/index.ts';
import userRepository from './repositories/userRepository.ts';
import chatService from './services/chatService.ts';
import type { Caller } from './services/appointmentService.ts';
import { toMessageView } from './controllers/chatController.ts';
import { verifyAccessToken } from './utils/tokens.ts';

/** Identity attached to a socket at handshake. Sockets never carry client-supplied ids. */
interface SocketAuth extends Caller {
  email: string;
}
...
const startRateLimitCleanup = (): NodeJS.Timeout => {
